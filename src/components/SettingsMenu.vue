...
<template>
  <!-- Settings Button -->
  <button popovertarget="settings-menu" class="fixed top-4 right-6 z-10 px-3 py-1 border cursor-pointer transition-colors"
    style="anchor-name: --settings-btn" :style="{
  ...
  <!-- Main Settings Menu -->
  <div id="settings-menu" popover class="m-0 p-0 border"
    style="position-anchor: --settings-btn; anchor-name: --settings-menu; inset: unset; top: anchor(bottom); right: anchor(right)" :style="{
      borderColor: theme.surface1,
      backgroundColor: theme.surface0,
    }">
    ...
    <button popovertarget="week-submenu"
      class="block w-full px-3 py-1 text-left cursor-pointer transition-colors" :style="{
        color: theme.text,
      }"
      @mouseenter="($event.target as HTMLElement).style.backgroundColor = theme.surface1"
  ...
  <!-- Theme Sub-menu -->
  <div id="theme-submenu" popover class="m-0 p-0 border"
    style="position-anchor: --settings-menu; inset: unset; top: anchor(top); right: anchor(left)" :style="{
      borderColor: theme.surface1,
      backgroundColor: theme.surface0,
    }">
    <button v-for="name in themeNames" :key="name" popovertarget="settings-menu"
      class="block w-full px-3 py-1 text-left cursor-pointer transition-colors" :style="{
  ...
  <div id="week-submenu" popover class="m-0 p-0 border"
    style="position-anchor: --settings-menu; inset: unset; top: anchor(bottom); right: anchor(left)" :style="{
      borderColor: theme.surface1,
      backgroundColor: theme.surface0,
    }">
    <button popovertarget="settings-menu"
      class="block w-full px-3 py-1 text-left cursor-pointer transition-colors whitespace-nowrap" :style="{
